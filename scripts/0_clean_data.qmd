...
This data was collected by Ph.D student Valentin Moser and Master students in 2021 and 2022 within 16 different streams in Switzerland. In each stream they sampled in 4 locations: `control`, `inflow`, `pool`, `outflow`. They sampled terrestrial arthropods by using a 5 x 1 m plots located at three diffferent poits: one meter from the stream's edge (1), at 2 times the pool width distance (2) and 3 times the pool width distance (3) @fig-suctiondesign. Within each plot, they sampled the arthropods at the two ends of the 5 x 1 m plot in cylindrical baskets (50 cm diameter, 67 cm height, woven fabric) using suction sampling on a sunny day between 10:00-17:00 during peak arthropod activity @fig-suction. The samples were stored in ethanol, individuals were counted, measured and identified to order level with the help of a binocular.
...
We are going to sub-sample the first and second point, close to the stream .
...
ggplot(t3, aes(y = location, x = length, fill = location)) +
  ...
  geom_text(
    data = stats,
    aes(x = max(t3$length) * .75,   # place text to the right of distributions
        y = location,
        label = label),
    inherit.aes = FALSE,
    hjust = 0
