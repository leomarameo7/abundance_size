...
site_info <- read_csv(here("data", "raw" , "site_info.csv")) |> select (c(2:5,8,9,19,23)) |> select(-c(site, sample))
...
# Alternative: side-by-side histogram (use 'dodge')
ggplot(m3, aes(x = size, fill = location)) +
  geom_histogram(binwidth = 1, color = "black", alpha = 0.7, position = "dodge") +
  labs(
    title = "Histogram of individual sizes by sampled location",
    x = "Size (mm)",
...
# with stat halfeye
ggplot(m3, aes(y = location, x = size, fill = location)) +
  stat_halfeye(
    position = "dodge",
    adjust = 1.1,       
    width = 0.6,        
