...
We are going to use the kick net data set, macroinvertebrates. Remember that we have used the `estimate_xmin()` function from the `poweRlaw` package [@gillespie2015] to estimate the lower value (xmin) from which the data start to follow the pareto distribution.
...
```{r testhypothesis}
# Test the overall null hypothesis that all lambdas are equal
hypothesis_test <- hypothesis(
  x = m1,
  hypothesis = c(
    "locationControl - locationInflow = 0",
    "locationControl - locationOutflow = 0",
    "locationControl - locationPool = 0"
  ),
  # Optional: You can specify 'class = "b"' but brms often infers it.
  # Optional: 'group' and 'dpar' may be needed if you had them, but here the coefficients are clear.
  # Optional: 'test' can be "none", "ci", or "ec" (evidence for change).
  # The default output gives a P-value (or similar Bayesian equivalent) and a Bayes Factor.
  # For a traditional hypothesis test, look at the probability in the 'Pr(H1)' column,
  # or the 'Evid.Ratio' (Bayes Factor).
)
...
# Define the ROPE boundaries
ROPE_LOWER <- -0.05
ROPE_UPPER <- 0.05

# -----------------------------------------------
# STEP 1: CREATE THE 'diff_draws' OBJECT
# -----------------------------------------------

# Assumes 'fit' is your fitted brms model object
diff_draws <- m1 %>%
  # Spread the draws to get coefficients into separate columns
  spread_draws(b_locationControl, b_locationInflow, b_locationOutflow, b_locationPool) %>%
  
  # Calculate all pairwise differences
  mutate(
    Inflow_vs_Control  = b_locationInflow - b_locationControl,
    Outflow_vs_Control = b_locationOutflow - b_locationControl,
    Pool_vs_Control    = b_locationPool - b_locationControl,
    
    Inflow_vs_Outflow  = b_locationInflow - b_locationOutflow,
    Pool_vs_Inflow     = b_locationPool - b_locationInflow,
    Pool_vs_Outflow    = b_locationPool - b_locationOutflow
  ) %>%
  
  # Convert to long format
  pivot_longer(
    cols = ends_with("_vs_Control") | ends_with("_vs_Inflow") | ends_with("_vs_Outflow"),
    names_to = "Comparison",
    values_to = "Difference"
  )

# -----------------------------------------------
# STEP 2: PREPARE DATA FOR FACETED ROPE PLOT
# -----------------------------------------------

control_diff_draws_rope <- diff_draws %>%
  # Filter only the comparisons against the Control group
  filter(Comparison %in% c("Inflow_vs_Control", "Outflow_vs_Control", "Pool_vs_Control")) %>%
  
  # Define ROPE Status for coloring the density slab
  mutate(
    ROPE_Status = case_when(
      Difference < ROPE_LOWER ~ "Outside ROPE (Lower)",
      Difference > ROPE_UPPER  ~ "Outside ROPE (Higher)",
      TRUE                    ~ "Inside ROPE"
    ),
    # Simplify comparison names for better plot titles/facets
    Comparison_Label = gsub("_vs_Control", " vs. Control", Comparison)
  )

# Calculate Probability Outside ROPE for Annotation
summary_data <- control_diff_draws_rope %>%
  group_by(Comparison_Label) %>%
  summarise(
    P_Outside = mean(Difference < ROPE_LOWER | Difference > ROPE_UPPER),
    # Find the maximum difference for positioning the text
    max_diff = max(Difference) 
  ) %>%
  mutate(
    prob_text = paste0("P(Outside ROPE) = ", label_percent(accuracy = 0.1)(P_Outside))
  )

# -----------------------------------------------
# STEP 3: GENERATE THE FACETED ROPE PLOT
# -----------------------------------------------

# -----------------------------------------------
# STEP 3: GENERATE THE FACETED ROPE PLOT (Corrected)
# -----------------------------------------------

ggplot(control_diff_draws_rope, aes(x = Difference, fill = ROPE_Status)) +
  
  # 1. Draw the ROPE as a background "bar" using geom_rect
  geom_rect(
  ...
  # 2. Density area (half-eye) with fill mapped to ROPE_Status
  stat_halfeye(
    aes(y = 0), # Map to a static Y-axis for a density plot
    .width = 0, 
    point_colour = NA
  ) +
  
  # 3. Add the null hypothesis line at 0
  geom_vline(xintercept = 0, linetype = "dashed", color = "darkred", linewidth = 1) +
  
  # 4. Add the text annotation with the calculated probability (using summary_data)
  geom_text(
    data = summary_data,
    aes(x = max_diff, y = Inf, label = prob_text),
    inherit.aes = FALSE, # <-- FIX: Prevents inheriting the 'fill = ROPE_Status' aesthetic
    hjust = 1.05,
    vjust = 1.5,
    size = 3.5,
    fontface = "bold",
    color = "black" # Optional: ensure the text color is always black
  ) +
  
  # 5. Facet by comparison
  facet_wrap(~ Comparison_Label, ncol = 1, strip.position = "left") +
  
  # Custom colors for ROPE status
  scale_fill_manual(
    values = c(
      "Outside ROPE (Lower)" = "#D55E00", 
      "Outside ROPE (Higher)" = "#0072B2", 
      "Inside ROPE" = NA 
    )
  ) +
  
  labs(
    x = "Posterior Difference (Treatment - Control)",
    y = "", 
    fill = "Probability Mass"
  ) +
  theme_minimal() +
  theme(
...
m2 = brm(formula = form , 
           ...
           iter = 1000,
           seed = 7, # allow exact replication of results 
          #file = here("results","models","m2_triggered"), # save the model. Please set your path folder. Note that if you want to refit the model, then you need to hide/deactivate this line of code 
        sample_prior = "no" )# options: "yes", "no" or "only". If you want to sample from priors, use ONLY. 
...
ggplot(pp_m2, aes(x= human_influence_z, y = .epred, group = location))+
  stat_lineribbon() 
    

```

Let's see conditional effects:
