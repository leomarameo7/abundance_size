...
k2 <- k %>% 
  mutate(across(c(human_influence), 
                ~ .x %>% scale() %>% as.vector(), 
                .names = "{.col}_z"),
         ecomorphology = as.factor(ecomorphology))
```
...
Now I am going to add the xmin and xmax column by site and location:
...
```{r estimate_xmin}
#  Compute xmin per group and summarize removal info
removal_summary <- k2 %>%
...
# just a dynamic data table to see what is going on: 
datatable(
  removal_summary,
  options = list(
...
::: callout-important
We trigger `r nrow(k2) - nrow(k3)` observations (`r round((nrow(k2) - nrow(k3)) / nrow(k2) * 100, 1)`%).

Remaining observations are:\
`r paste(k3 %>% count(location) %>% mutate(txt = paste0(location, " = ", n)) %>% pull(txt), collapse = "; ")`.
:::
...
# formula 
form = bf(dw | vreal(counts, xmin, xmax) ~ 0  + location, # 0 + allows to see the four locations with its own lambda . if you do x ~ 1  + location, then control location will consider your reference level. Try by yourself . 
           family = paretocounts())
...
default_prior(object = form, data = k4 )
...
validate_prior(prior = pr, formula = form, data = k4 )
...
m1 = brm(formula = form , 
          ...
          file = here("results","models","m1"), # save the model. Please set your path folder. Note that if you want to refit the model, then you need to hide/deactivate this line of code 
        sample_prior = "yes" )# yes, no or only
```
...
ggplot(data = pp, aes(y = location, x = .epred, fill = location)) + 
  ...
  labs(x = expression(lambda),
     ...
     fill = "location")+
  theme_bw(base_size = 14)+ 
  scale_x_continuous(breaks = seq(-1.25, -0.75, by = 0.1))
```
