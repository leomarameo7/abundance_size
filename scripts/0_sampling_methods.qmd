...
# Run the check
inconsistent_families <- check_family_order_consistency(a)
...
a2 <- a1 %>%
  mutate(
    a = case_when(
      is.na(a) & family %in% g$family & order %in% g$order ~ g$a[match(paste(family, order), paste(g$family, g$order))],
      TRUE ~ a
    ),
    b = case_when(
      is.na(b) & family %in% g$family & order %in% g$order ~ g$b[match(paste(family, order), paste(g$family, g$order))],
      TRUE ~ b
    ),
    source = case_when(
      is.na(source) & family %in% g$family & order %in% g$order ~ g$source[match(paste(family, order), paste(g$family, g$order))],
      TRUE ~ source
    ),
    dry_weight = ifelse(is.na(dry_weight), a * (length ^ b), dry_weight)
  )
