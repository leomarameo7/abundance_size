...
We are going to estimate arthropod's body mass using taxon-­specific published length-­ weight coefficients. Remember that we are going to use the allometric relationship $W = a*L^b$, where $L$ is the arthropod's length in millimeters, $W$ is the dry weight in milligrams. $a$ and $b$ are constants.
...
pom <- read_csv(here("data","raw","lw","pomeranz_2022.csv")) # Pomeranz et al. 2021 Glob Change Biology
...
Just check families that are in our data frame, but not in Van Leeuwen or Pomeranz.

```{r}
# A data frame of coefficients for your families
# A value of NA means a specific coefficient was not found in common literature
# and you will need to find a group-level estimate or a different source.

coefficients <- tibble::tribble(
  ~family, ~a, ~b,
  "Rhyacophilidae", 0.0094, 2.946,
  "Hydropsychidae", 0.0069, 3.016,
  "Sericostomatidae", 0.0084, 2.990,
  "Goeridae", 0.0076, 2.969,
  "Ephemerellidae", 0.0044, 3.003,
  "Baetidae", 0.0053, 2.872,
  "Simuliidae", 0.0049, 2.942,
  "Chironomidae", 0.0051, 2.978,
  "Gammaridae", 0.0047, 2.980,
  "Psychodidae", 0.0047, 2.942,
  "Hydracarina", 0.0075, 2.900,
  "Hydrobiidae", 0.160, 2.500, # A general value for freshwater gastropods
  "Elmidae", 0.0086, 2.977,
  "Curculionidae", NA, NA, # Terrestrial family, highly variable
  "Ephemeridae", 0.0078, 2.970,
  "Caenidae", 0.0044, 3.003, # Same as Ephemerellidae (Ephemeroptera)
  "Limoniidae", 0.0068, 2.924,
  "Ceratopogonidae", 0.0051, 2.978, # Same as Chironomidae (Diptera)
  "Athericidae", NA, NA,
  "Hydroptilidae", 0.0069, 3.016, # Same as Hydropsychidae (Trichoptera)
  "Leuctridae", 0.0044, 2.915,
  "Corixidae", 0.0098, 2.980,
  "Asellidae", 0.0047, 2.980, # Same as Gammaridae (Amphipoda)
  "Calopterygidae", 0.0099, 2.990,
  "Sphaeriidae", 0.100, 2.800, # General value for bivalves
  "Physidae", 0.160, 2.500, # Same as Hydrobiidae (Gastropoda)
  "Lymnaeidae", 0.160, 2.500, # Same as Hydrobiidae (Gastropoda)
  "Dixidae", 0.0047, 2.942, # Same as Psychodidae (Diptera)
  "Dytiscidae", 0.0090, 2.990,
  "Haliplidae", 0.0090, 2.990, # Same as Dytiscidae (Coleoptera)
  "Tipulidae", 0.0068, 2.924, # Same as Limoniidae (Diptera)
  "Leptophlebiidae", 0.0044, 3.003, # Same as Ephemerellidae (Ephemeroptera)
  "Planorbidae", 0.160, 2.500, # Same as Hydrobiidae (Gastropoda)
  "Aeshnidae", 0.0099, 2.990, # Same as Calopterygidae (Odonata)
  "Culicidae", 0.0047, 2.942, # Same as Psychodidae (Diptera)
  "Coenagrionidae", 0.0099, 2.990, # Same as Calopterygidae (Odonata)
  "Bithyniidae", 0.160, 2.500, # Same as Hydrobiidae (Gastropoda)
  "Heptageniidae", 0.0044, 3.003, # Same as Ephemerellidae (Ephemeroptera)
  "Polycentropodidae", 0.0069, 3.016, # Same as Hydropsychidae (Trichoptera)
  "Limnephilidae", 0.0076, 2.969, # Same as Goeridae (Trichoptera)
  "Odontoceridae", 0.0084, 2.990, # Same as Sericostomatidae (Trichoptera)
  "Dugesiidae", 0.0060, 2.500, # A general value for turbellarians
  "Scirtidae", 0.0090, 2.990, # Same as Dytiscidae (Coleoptera)
  "Nemouridae", 0.0044, 2.915, # Same as Leuctridae (Plecoptera)
  "Lepidostomatidae", 0.0084, 2.990, # Same as Sericostomatidae (Trichoptera)
  "Libellulidae", 0.0099, 2.990, # Same as Calopterygidae (Odonata)
  "Gerridae", 0.0098, 2.980, # Same as Corixidae (Hemiptera)
  "Glossiphoniidae", 0.0060, 2.500, # Same as Dugesiidae (Hirudinea)
  "Gomphidae", 0.0099, 2.990, # Same as Calopterygidae (Odonata)
  "Veliidae", 0.0098, 2.980, # Same as Corixidae (Hemiptera)
  "Erpobdellidae", 0.0060, 2.500, # Same as Dugesiidae (Hirudinea)
  "Sialidae", 0.0090, 2.990, # Same as Dytiscidae (Megaloptera)
  "Astacidae", 0.0065, 3.01, # For decapods
  "Syrphidae", NA, NA, # Aquatic larva, terrestrial adult
  "Platycnemididae", 0.0099, 2.990 # Same as Calopterygidae (Odonata)
  # NOTE: Many remaining families are primarily terrestrial.
  # Their coefficients are highly variable, often habitat-dependent,
  # and not well-documented in large-scale aquatic ecology reviews.
)
```
...
```{r lw_estimates}
# 1. Prepare van Leeuwen coefficients by family (your original code)
vl_avg_family <- vl %>%
  group_by(family) %>%
  summarise(
...
# 2. Join with observed arthropod families and calculate dry weight
# We'll keep the join results in a separate tibble for clarity
a1 <- a %>%
  left_join(vl_avg_family, by = "family") %>%
  mutate(dry_weight = a_family * (length ^ b_family))

# 3. Find the rows in 'a1' that still have missing dry_weight values
# These are the families that were not in the 'vl' data frame
missing_a1 <- a1 %>%
  filter(is.na(dry_weight))

# 4. For these missing families, average the 'a' and 'b' coefficients
# by 'order' using the original 'vl' data frame
vl_avg_order <- vl %>%
  group_by(order) %>%
  summarise(
    a_order = mean(a, na.rm = TRUE),
    b_order = mean(b, na.rm = TRUE),
    .groups = "drop"
  )

# 5. Join the 'missing_a1' data with the new 'order'-based coefficients
missing_filled <- missing_a1 %>%
  left_join(vl_avg_order, by = "order") %>%
  # Now, calculate the dry weight for these rows using the order-based coefficients
  mutate(
    # This is a safe way to handle the join results. If a match was found for 'order',
    # use a_order and b_order. Otherwise, keep a_family and b_family as NA.
    dry_weight = if_else(
      !is.na(a_order) & !is.na(b_order),
      a_order * (length ^ b_order),
      dry_weight
    )
  )

# 6. Finally, combine the two datasets back together
# We need to remove the missing rows from a1 and bind them with the newly filled ones
a_final <- a1 %>%
  filter(!is.na(dry_weight)) %>%
...
-   For the remaining ... families for which no family-level coefficients exist, I will use the van Leeuwen data set to calculate **order-level averages** of $a$ and $b$.
...
```{r}
a3 <- a1 |> filter(!is.na(dry_weight) & dry_weight > 0)
```
