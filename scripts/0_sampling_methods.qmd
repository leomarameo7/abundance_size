...
We are going to estimate arthropod's body mass using taxon-­specific published length-­ weight coefficients. Remember that we are going to use the allometric relationship $W = a*L^b$, where $L$ is the arthropod's length in millimeters, $W$ is the dry weight in milligrams. $a$ and $b$ are constant coefficients.
...
# --- 1. Fix datasets ---
pom <- pom %>%
  mutate(family = ifelse(family == "Helicopsychidae (case width)", "Helicopsychidae", family))
...
# --- 3. First join with vl_avg (priority source) ---
a <- a %>%
  left_join(vl_avg, by = "family") %>%
  mutate(dry_weight = a * (length ^ b))

# --- 4. Find families still missing after vl ---
missing_fams <- a %>%
  filter(is.na(dry_weight)) %>%
  distinct(family)

# --- 5. Complement with pom_avg for missing families ---
a <- a %>%
  left_join(
    pom_avg %>% filter(family %in% missing_fams$family),
    by = "family",
    suffix = c("", "_pom")
  ) %>%
  mutate(
    a = ifelse(is.na(a), a_pom, a),
    b = ifelse(is.na(b), b_pom, b),
    dry_weight = ifelse(is.na(dry_weight), a * (length ^ b), dry_weight)
  ) %>%
  select(-a_pom, -b_pom)

# --- 6. Check if anything is still missing ---
still_missing <- a %>%
  filter(is.na(dry_weight)) %>%
  distinct(family)

cat("Number of families still missing after vl + pom:", nrow(still_missing), "\n")
print(still_missing)

```

```{r}
# --- Step 1: Prepare order-level averages from vl + pom ---
vl_order <- vl %>%
  group_by(order) %>%
  summarise(
    a = mean(a, na.rm = TRUE),
    b = mean(b, na.rm = TRUE),
    source = "van Leeuwen (order-level)",
    .groups = "drop"
  )
...
pom_order <- pom %>%
  ...
  summarise(
    a = mean(a, na.rm = TRUE),
    b = mean(b, na.rm = TRUE),
    source = "Pomeranz (order-level)",
    .groups = "drop"
  )

# --- Step 2: Prioritise vl_order, then fill remaining from pom_order ---
a <- a %>%
  left_join(vl_order, by = "order", suffix = c("", "_order_vl")) %>%
  mutate(
    a = ifelse(is.na(a), a_order_vl, a),
    b = ifelse(is.na(b), b_order_vl, b),
    dry_weight = ifelse(is.na(dry_weight), a * (length ^ b), dry_weight)
  ) %>%
  select(-a_order_vl, -b_order_vl)

# Step 3: Complement with pom_order if still missing
a <- a %>%
  left_join(pom_order, by = "order", suffix = c("", "_order_pom")) %>%
  mutate(
    a = ifelse(is.na(a), a_order_pom, a),
    b = ifelse(is.na(b), b_order_pom, b),
    dry_weight = ifelse(is.na(dry_weight), a * (length ^ b), dry_weight)
  ) %>%
  select(-a_order_pom, -b_order_pom)

# --- Step 4: Report what’s still missing ---
still_missing <- a %>%
  filter(is.na(dry_weight)) %>%
  distinct(order, family)

cat("Number of families still missing after order-level fill:", nrow(still_missing), "\n")
print(still_missing)

```

```{r}
nas <- a |> filter(is.na(dry_weight))
```
...
# Assuming your dataframe is named 'a' and has columns 'order', 'family', and a column for length (e.g., 'length_mm')
# Make sure to replace 'your_length_column' with the actual name of your length column.

# Step 1: Assign a and b coefficients based on order/family
a <- a %>%
  mutate(
    a_coeff = case_when(
      order %in% c("Arachnida", "Araneae", "Ixodida", "Acari", "Opiliones") ~ 0.035,
      order == "Gastropoda" ~ 0.28,
      order == "Heteroptera" ~ 0.054,
      order %in% c("Hymenoptera", "Mecoptera", "Neuroptera") ~ 0.007,
      order == "Dermaptera" ~ 0.007,
      order == "Zygentoma" ~ 0.002,
      order == "Orthoptera" ~ 0.005,
      TRUE ~ 0.005 # Default coefficient for all other orders
    ),
    b_coeff = case_when(
      order %in% c("Arachnida", "Araneae", "Ixodida", "Acari", "Opiliones") ~ 2.6,
      order == "Gastropoda" ~ 2.8,
      order == "Heteroptera" ~ 2.62,
      order %in% c("Hymenoptera", "Mecoptera", "Neuroptera") ~ 2.62,
      order == "Dermaptera" ~ 2.92,
      order == "Zygentoma" ~ 3.42,
      order == "Orthoptera" ~ 2.89,
      TRUE ~ 2.89 # Default coefficient for all other orders
    )
  )

# Step 2: Estimate the dry_weight for rows where it is missing
# This step uses your defined a and b coefficients to calculate dry weight
# Replace 'your_length_column' with the actual name of your column containing body length.
a <- a %>%
  mutate(
    dry_weight = ifelse(is.na(dry_weight), a_coeff * (length^b_coeff), dry_weight)
  )

# --- Step 3: Check for remaining missing values ---
still_missing_after_fill <- a %>%
  filter(is.na(dry_weight)) %>%
  distinct(order, family)

cat("\nNumber of families still missing after allometric fill:", nrow(still_missing_after_fill), "\n")
print(still_missing_after_fill)
```
