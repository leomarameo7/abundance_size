...
In order to answer these questions, we have sampled the arthropod community in Swiss streams dammed by beavers. We have used three (3) different methods, catching different insect taxons. The three methods from now on we defined as: `intercept_trap`, `kick_net`, `suction`.
...
ggplot(d, aes(x = length, fill = location)) +
  ...
  labs(
    title = "Histogram of individual sizes by sampled location (intercept trap)",
    x = "Size (mm)",
    y = "Frequency",
    fill = "Location"
  ) +
...
ggplot(d, aes(y = location, x = length, fill = location)) +
  ...
  labs(
    title = "Size Distributions by Location (intercept trap)",
    y = "Location",
    x = "Size (mm)"
  ) +
  theme_bw(base_size = 14)
