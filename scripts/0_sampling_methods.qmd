...
fams_not_in_pz <- setdiff(fams$family, lw_pomz$taxon)
```
...
### add coefficients to our df from Pomeranz et al. 2022 ####

dat_lw <- m4 |>
  left_join(lw_narrow, by = join_by(family == taxon))
```
...
dat_dw <- dat_lw |>
  mutate(dw = case_when(formula_type == 1 ~ a * length^b,
                        formula_type == 2 ~ exp(a + b * log(length))))
```
...
write.csv(dat_dw_1, here("data","processed", "kicknet_clean.csv" ))
