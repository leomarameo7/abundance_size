...
::: callout-important
What it is important to remember is that the Pareto distribution is using the individual body sizes to predict the shape of the decline in frequency from small to large individuals. 
:::

### About binning

| [Jeff Wesner](https://scholar.google.com/citations?hl=en&user=1J-5_yUAAAAJ&view_op=list_works&sortby=pubdate): However, when we have individual measures of body size, each size is technically unique in the sense that no two masses are going to be identical down to the 4^th^ decimal or so. For that reason, there is no variation in abundance; i.e., the abundance of each individual mass = 1. This is why past methods used binning...because if we sum up a bunch of 1's in bins of mass, then we have a response variable that we can fit with linear or non-linear regressions as $abundance_{bin} \sim bin^b$. But as we see in [@pomeranz2024] and others, binning produces a lots of problems, resulting in poor estimates of the exponent. With the Pareto, what we're modeling is not $abundance \sim mass^b$, but $f(mass) \sim mass^b$. That is, the *frequency* of mass as a function of mass. But $f(mass)$ is not measured, it us estimated through the Pareto distribution. 
...
### Considerations about undersampling small body masses

| [Jeff Wesner](https://scholar.google.com/citations?hl=en&user=1J-5_yUAAAAJ&view_op=list_works&sortby=pubdate): For the package `isdbayes` , we used the bounded Pareto distribution because [@edwards2017] had shown that it was the best option for fitting size spectra. Pareto distribution assumes that abundance always decreases with size. But in most cases with real data, small things are harder to measure and they get undercounted. By itself, that means that the data do not follow a Pareto distribution. As [@clauset2009] explain, unless we have perfectly sampled data (which is impossible), then **our goal is to first find the range of data that *do* follow a power law distribution, which in turn means relying on some sort of cutoff/algorithm**. After that, then we choose a power law distribution (i.e. Pareto distribution in the `isdbayes` package) and go with it. But the type of distribution we choose in the second step is irrelevant to the first step.  
...
This session will be at Eawag (FX F 13) with Leonardo Capitani. You should use these 3.45 hours to:

-   read instruction html: section *Considerations about undersampling small body masses*

-   fit the model of the last session to the `kicknet_triggered.csv` dataset
